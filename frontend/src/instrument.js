/**
 * Sentry: на iPhone/iPad отключён. На десктопе — только после согласия на аналитические cookie.
 */
...
import { hasAnalyticsConsent } from './lib/cookieConsent.js';
import { DOM_GLITCH_RE } from './lib/domGlitch.js';
...
function applySentryModule(SentryMod) {
  Sentry = SentryMod;
  SentryMod.init({
    dsn,
    ...
    beforeSend(event) {
      ...
      if (DOM_GLITCH_RE.test(String(message))) return null;
      return event;
    },
  });
}

export function bootSentryAfterConsent() {
  if (sentryOff || sentryBooted || !hasAnalyticsConsent()) return;
  sentryBooted = true;
  import('@sentry/react').then(applySentryModule).catch(() => {});
}

if (!sentryOff && typeof window !== 'undefined' && hasAnalyticsConsent()) {
