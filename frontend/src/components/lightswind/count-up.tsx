...
export function CountUp({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        'inline-flex items-center justify-center text-inherit font-bold',
        className
      )}
    >
      <motion.div
        {...getHoverAnimation()}
        className={cn('flex items-center text-inherit transition-all', colorClass, numberClassName)}
        style={colorScheme === 'custom' && customColor ? { color: customColor } : undefined}
      >
        {prefix && <span className="mr-1 text-inherit">{prefix}</span>}
        <motion.span className="text-inherit">{rounded}</motion.span>
