...
function ConfirmChannelCard({ channel, selected, onSelect }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: '14px 16px',
        borderRadius: '18px',
        border: `2px solid ${isSelected ? PREMIUM.accent : PREMIUM.borderLight}`,
        background: isSelected ? PREMIUM.accentSoft : PREMIUM.bgSoft,
        cursor: 'pointer',
        textAlign: 'left',
...
// ============================================================
function PageSpinner({ size = 48, label }) {
  return (
    <div style={{ textAlign: 'center' }}>
      <div
        style={{
          width: size,
          height: size,
          border: '4px solid rgba(106, 90, 205, 0.15)',
          borderTopColor: PREMIUM.accent,
          borderRadius: '50%',
          animation: 'spin 0.8s linear infinite',
          margin: label ? '0 auto 16px' : '0 auto',
        }}
      />
      {label ? (
        <p style={{ fontSize: '15px', fontWeight: '600', color: PREMIUM.textSecondary, margin: 0 }}>
          {label}
        </p>
      ) : null}
    </div>
...
export default function ClientPage() {
  ...
  const masterSeoHead = !isBookingApp && master && pageSeo ? (
    <>
      <SeoHead
        title={pageSeo.title}
    ...
    return (
      ...
      >
        {loadTimedOut ? (
          ...
          <PageSpinner label="Загрузка страницы…" />
        )}
      </div>
    );
  }
                        ...
                        <div style={{ display: 'grid', gap: '16px', gridTemplateColumns: '1fr' }}>
                          <div>
                            ...
                            <div className="premium-calendar-wrap" style={{ borderRadius: '20px', overflow: 'hidden' }}>
                              <Suspense fallback={<PageSpinner size={36} label="Загрузка календаря…" />}>
                                <BookingDateCalendar
                      ...
                      <PremiumCard style={{ padding: '20px' }}>
                        ...
                        {!rescheduleId && confirmPhase === 'choose' && (
                          <div style={{ marginTop: '20px' }}>
                            <p style={{ fontSize: '14px', fontWeight: '700', color: PREMIUM.textPrimary, marginBottom: '4px' }}>
                              Как подтвердить запись?
                            </p>
                            <p style={{ fontSize: '13px', color: PREMIUM.textSecondary, marginBottom: '14px', lineHeight: 1.45 }}>
                              В Telegram и MAX — кнопка в боте. На почту — код из письма.
                            </p>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        ...
                        <div style={{ marginTop: '16px' }}>
                          {rescheduleId ? (
                            ...
                            <PremiumCtaBtn
                              type="submit"
                              disabled={!canSubmitBooking(formData) || !confirmChannel || (confirmChannel === 'email' && !confirmEmail.trim())}
                              loading={booking}
                            >
