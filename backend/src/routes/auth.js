  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO masters (id, email, password_hash, name, last_name, salon_name, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) RETURNING id, email, name, last_name, salon_name`,
...
router.get('/verify', async (req, res) => {
  try {
    ...
    const result = await db.query('SELECT id, email, name, last_name, salon_name FROM masters WHERE id = $1', [decoded.masterId]);
    if (result.rows.length === 0) {
      return res.status(401).json({ valid: false });
    }
