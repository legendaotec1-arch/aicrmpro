...
function generateUniqueMeta(slug, ctx) {
  ...
  // никогда не пересекались с нишевыми. Это автоматически снижает дубли.
  ...
  // Дополнительная вариативность: варьируем salt в pickOne по niche.id,
  // чтобы разные ниши не пересекались на одном индексе шаблона.
  const nicheSalt = ctx?.nicheId || (slug.match(/^[a-z-]+/) || ['x'])[0];
  // Берём шаблон и ДО подстановки запоминаем его индекс — для выбора суффикса
  const templateIdx = hashNum(slug, `meta:${nicheSalt}`) % pool.length;
  const template = pool[templateIdx];
  let meta = fill(template, vars);
  // Всегда добавляем уникальный slug-привязанный суффикс из META_TAIL_PHRASES,
  // чтобы одинаковые шаблоны не давали одинаковый description для разных страниц.
  // Хеш от slug стабильный — для одного и того же слага всегда один и тот же
  // суффикс, но между разными слагами распределение равномерное.
  const h = crypto.createHash('sha256').update(`meta-tail:${slug}`).digest();
  const tailIdx = h.readUInt16BE(0) % META_TAIL_PHRASES.length;
  const tail = META_TAIL_PHRASES[tailIdx];
  // Если шаблон уже заканчивается точкой, ставим суффикс через пробел.
  meta = /[.!?]\s*$/.test(meta) ? `${meta} ${tail}` : `${meta}. ${tail}`;
  return meta;
}
